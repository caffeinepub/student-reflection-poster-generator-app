...
import { useGetReflections, useSaveReflection, useUpdateReflection } from '../hooks/queries/useReflections';
...
import { Plus, Save, FileText, Sparkles } from 'lucide-react';
import { toast } from 'sonner';
import type { Reflection } from '../backend';
import ImproveReflectionPanel from '../components/ImproveReflectionPanel';
import { loadDraft, saveDraft, clearDraft } from '../lib/reflectionDraftStore';
    ...
    <div className="grid lg:grid-cols-3 gap-6">
      <Card className="lg:col-span-1">
        <CardHeader>
          ...
          <CardDescription>Select a reflection to edit or create a new one</CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-medium truncate">{reflection.title}</div>
      ...
      <Card className="lg:col-span-2">
        <CardHeader>
          <CardTitle>{isEditing ? 'Edit Reflection' : 'New Reflection'}</CardTitle>
          <CardDescription>Write your thoughts and learning experiences</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Button onClick={handleSave} disabled={saveReflection.isPending || updateReflection.isPending} className="flex-1">
              <Save className="h-4 w-4 mr-2" />
              {saveReflection.isPending || updateReflection.isPending ? 'Saving...' : 'Save Reflection'}
            </Button>
            <Button
              variant="outline"
          ...
          {showImprove && body.trim() && (
            <ImproveReflectionPanel reflectionText={body} onApply={handleApplyImprovement} />
          )}
        </CardContent>
      </Card>
    </div>
