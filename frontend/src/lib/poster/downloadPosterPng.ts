...
export async function downloadPosterPng(reflection: Reflection, template: Template): Promise<void> {
  ...
  const gradientBottom = ctx.createLinearGradient(0, rect.height * 0.7, 0, rect.height);
  ...
  canvas.toBlob((blob) => {
    ...
    const sanitizedTitle = reflection.title.replace(/[^a-z0-9]/gi, '-').toLowerCase();
