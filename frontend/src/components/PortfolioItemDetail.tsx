...
import { Calendar, X, Download, Copy } from 'lucide-react';
...
export default function PortfolioItemDetail({ item, onClose }: PortfolioItemDetailProps) {
    ...
    <Card>
        ...
        <div className="flex items-start justify-between">
          <div className="space-y-1">
            ...
            <CardDescription className="flex items-center gap-2">
              ...
              {new Date(Number(item.postTimestamp) / 1_000_000).toLocaleDateString()}
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="text-sm text-muted-foreground">{item.template.name}</div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="p-3 bg-muted rounded-lg text-sm whitespace-pre-wrap">{postText}</div>
        </div>
